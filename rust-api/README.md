...
S3 업로드용 Presigned URL을 발급하는 초경량 Rust API 서비스입니다.
...
## ☁️ 3. AWS ECS/EKS 배포 준비

### Step 1: 리소스 준비 (최초 1회)

아래 명령어들을 순서대로 실행하여 필요한 AWS 리소스를 생성합니다.
...
EKS 및 로드밸런서 연동을 위한 보안 그룹을 생성합니다.

```bash
# VPC ID 확인 (기존 클러스터 사용 시)
export VPC_ID="vpc-067604da147fbd40a"
...
> **참고**: ECS Fargate로 배포를 원하시면 [ECS 배포 가이드(ECS_GUIDE.md)](ECS_GUIDE.md) 문서를 참고하세요.

---

## ☸️ 4. Kubernetes (EKS) 배포

`eks_yaml/rust-api/` 폴더의 매니페스트를 사용합니다.

1.  `configmap.yaml`에서 S3 버킷명 등 설정 수정
2.  `deployment.yaml`에서 이미지 주소(`<ACCOUNT_ID>`) 수정
3.  `nginx-web/service.yaml`에서 `aws-load-balancer-security-groups`에 위에서 생성한 **LB_SG_ID** 입력
4.  배포 실행:
    ```bash
